...
--? Find the lowest list price of all products:
 SELECT  MAX(unitprice) max_list_price,MIN(unitprice) min_list_price 
 FROM products;
...
SELECT customerid,year(orderdate) order_year, count(orderdate)
FROM orders
WHERE customerid like('A%') group by customerid,year(orderdate)
ORDER BY customerid;
...
SELECT YEAR (orderdate) order_year,customerid, count(orderdate) CountOfOrdersPlaced 
FROM orders
WHERE customerid like 'A%' GROUP BY customerid, YEAR (orderdate) 
ORDER BY customerid,3 desc
--CountOfOrdersPlaced desc;



--? Return the number of customers in every city:
SELECT city, COUNT (customerid) customer_count FROM
customers GROUP BY city ORDER BY customer_count desc;
--? Return number of customers by region and city:
SELECT region,city, COUNT (customerid) customer_count FROM
customers GROUP BY region, city ORDER BY  region desc,city;
...
--? Find the customers who placed at least 25 orders in total:
...
SELECT orderid,(quantity * unitprice * (1 - discount)) net_value
FROM "order details"
...
FROM products
GROUP BY categoryid
HAVING MAX (unitprice) > 4000 OR MIN (unitprice) < 500
order by categoryid
...
--Should be able to describe relationships between tables
--and write simple queries to retrieve data from the database
__________________________Stage1 SQL Contents Complete ___________________________________
